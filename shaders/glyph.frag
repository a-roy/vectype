...
{
    vec4 tex = texelFetch(uCurvesSampler, vCurvesMin + coordIndex);
    p[0] = tex.ba - vNormCoord;

    tex = texelFetch(uCurvesSampler, vCurvesMin + coordIndex + 1);
    p[1] = tex.rg - vNormCoord;
    p[2] = tex.ba - vNormCoord;
}
...
void main()
{
    ...
    ivec2 indicesCoord = ivec2(vGridMin + integerCell);
    vec2 cellMid = (integerCell + 0.5) / vGridSize;

    mat2 initrot = inverse(mat2(
        dFdx(vNormCoord) * kPixelWindowSize,
        dFdy(vNormCoord) * kPixelWindowSize));
    ...
    indices1 = ivec4(texelFetch(uAtlasSampler, indicesCoord));
    indices2 = ivec4(texelFetch(
        uAtlasSampler,
        ivec2(indicesCoord.x + vGridSize.x, indicesCoord.y)));
            ...
            for (int tindex = 0; tindex < 2; tindex++)
            {
                ...
                if (t[tindex] > 0.0 && t[tindex] <= 1.0)
                {
                    ...
                    if (posy > -1.0 && posy < 1.0)
                    {
                        ...
                        // The wrong operation will give buggy looking results, not a simple
                        // inverse.
    ...
    for (int ss = 0; ss < numSS; ss++)
    {
        if ((firstIntersection[ss] >= 2.0 && midVal) ||
            (firstIntersection[ss] > 0.0 && abs(firstIntersection[ss]) < 2.0))
        {
